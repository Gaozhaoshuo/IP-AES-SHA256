...
module apb_ms_model (
  ...
  parameter IFNAME = "../Src/testcase/aes_enc_128b_ecb_trc.bin";
  ...
  // debug dump control (only print first several lines to avoid huge logs)
  integer dbg_line;
  integer dbg_max_lines;
  initial dbg_max_lines = 8;  // 打印前 8 行（每行16B），想全打就设很大
      integer back_off;
  initial begin
    ...
    if (enc_dec) $display("Encrypt case.");
    else $display("Decrypt case.");

    if (key_len == 2) $display("256 bit key length");
    else $display("128 bit key length");
    ...
    for (i = 0; i < bulk_num; i = i + 1) begin
      ...
      if (!enc_dec)  //decrypt mode
        void'($fseek(in_fp, in_byte_len, 1));  //go to code position
      ...
      back_off = -(2 * in_byte_len);
      if (!enc_dec) void'($fseek(in_fp, back_off, 1));
      ...
      for (j = 0; j < in_byte_len; j = j + 1) begin
        ...
        // ---- debug print: 每 16B 打一行（Ref 与 DUT）
        if ((j[3:0] == 4'h0) && (dbg_line < dbg_max_lines)) begin
          $write("BULK %0d, off 0x%08x : REF =", i, j);
        end

        if (dbg_line < dbg_max_lines) begin
          $write(" %02x", tmp8);
        end

        if ((j[3:0] == 4'hf) && (dbg_line < dbg_max_lines)) begin
          $write(" | DUT =");
          // 同一行把 DUT 的 16 字节也打出来
          $write(" %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x",
                 `SYS_MEM_PATH[`OUT_DATA_BASE+(j-15)], `SYS_MEM_PATH[`OUT_DATA_BASE+(j-14)],
                 `SYS_MEM_PATH[`OUT_DATA_BASE+(j-13)], `SYS_MEM_PATH[`OUT_DATA_BASE+(j-12)],
                 `SYS_MEM_PATH[`OUT_DATA_BASE+(j-11)], `SYS_MEM_PATH[`OUT_DATA_BASE+(j-10)],
                 `SYS_MEM_PATH[`OUT_DATA_BASE+(j-9)], `SYS_MEM_PATH[`OUT_DATA_BASE+(j-8)],
                 `SYS_MEM_PATH[`OUT_DATA_BASE+(j-7)], `SYS_MEM_PATH[`OUT_DATA_BASE+(j-6)],
                 `SYS_MEM_PATH[`OUT_DATA_BASE+(j-5)], `SYS_MEM_PATH[`OUT_DATA_BASE+(j-4)],
                 `SYS_MEM_PATH[`OUT_DATA_BASE+(j-3)], `SYS_MEM_PATH[`OUT_DATA_BASE+(j-2)],
                 `SYS_MEM_PATH[`OUT_DATA_BASE+(j-1)], `SYS_MEM_PATH[`OUT_DATA_BASE+(j-0)]);
          $write("\n");
          dbg_line = dbg_line + 1;
        end

        // ---- 原有功能：逐字节比较，一旦 mismatch 就 fail（不改）
      ...
      // 如果本 bulk 不是 16B 对齐（理论上应是），补打印最后不足 16B 的那行（可选）
      if ((in_byte_len[3:0] != 4'h0) && (dbg_line < dbg_max_lines)) begin
        $write("\n");
      end


      if (!enc_dec) void'($fseek(in_fp, in_byte_len, 1));  //go to blk_len position of next bulk
    end
